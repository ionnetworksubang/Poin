...
// 1. SINKRONISASI DATA DAN MATIKAN LOADING SCREEN (VERSI V2)
async function fetchODPData() {
    try {
        // Panggil API V2 menggunakan metode POST
        ...
        if (result.ok) {
            // Ubah format mentah V2 agar bisa dibaca form Laporan
            let rawOdp = result.data.odp;
            let mapping = {};
            
            rawOdp.forEach(row => {
                let namaOdp = row[0]; // Kolom Nama ODP
                let namaOlt = row[2]; // Kolom Nama OLT
