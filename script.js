...
// ==========================================
// JURUS ANTI LEMOT (CACHE FIRST STRATEGY)
// ==========================================
async function fetchData() {
  // 1. Cek apakah ada data lama di memori HP
  let cachedData = localStorage.getItem('ionMasterData');
  
  if (cachedData) {
      // Jika ada, LANGSUNG TAMPILKAN tanpa loading panjang!
      masterData = JSON.parse(cachedData);
      populateOltDropdowns(); renderOdp(); renderOlt(); renderMember();
      document.getElementById('loginScreen').classList.add('hidden'); 
      document.getElementById('appScreen').classList.remove('hidden'); 
      hideLoader(); 
  } else {
      // Jika baru pertama kali banget install/buka, baru munculkan loading
      showLoader("Menyiapkan aplikasi...");
  }

  // 2. Sedot data terbaru dari Google Sheets di latar belakang (tanpa ganggu UI)
  try {
    let res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: "getData" }) }).then(r=>r.json());
    if (res.ok) {
      masterData = res.data; 
      
      // Simpan data terbaru ke memori HP untuk dibuka besok
      localStorage.setItem('ionMasterData', JSON.stringify(masterData)); 
      
      // Render ulang layar secara diam-diam agar data yang tampil adalah yang paling update
      populateOltDropdowns(); renderOdp(); renderOlt(); renderMember();
      ...
      // Kalau internet putus tapi ada cache, aplikasi tetap bisa dipakai!
      if (!cachedData) { 
          hideLoader(); 
...
// ========================================================
// FITUR BARU: UPLOAD MASAL DENGAN SMART ODP MATCHER
// ========================================================
...
async function processBulkLaporan() {
    ...
    let errorCount = 0;
    let odpNotFound = []; // Array untuk melacak jika ada ODP yg tidak dikenali sama sekali
    ...
    blocks.forEach(block => {
        ...
        let rawOlt = extract(block, "OLT");
        let rawOdp = extract(block, "ODP");
        ...
        if (nama && cid) {
            // --- SMART LOGIC: BACA NAMA ODP & OLT DARI DATABASE ---
            let finalOdp = rawOdp;
            let finalOlt = rawOlt;
            let isOdpFound = false;

            if (rawOdp) {
                let searchStr = rawOdp.toLowerCase().trim();
                
                // Cari ODP di masterData yang "mengandung" kata yang diketik
                let matchedRow = masterData.odp.find(r => r[0].toLowerCase().includes(searchStr));

                if (matchedRow) {
                    finalOdp = matchedRow[0]; // Pakai nama Full asli dari DB (Cth: ODP-KLJT...)
                    finalOlt = matchedRow[2]; // Overwrite OLT dengan OLT asli dari DB (Biar 100% presisi)
                    isOdpFound = true;
                }
            }

            if (!isOdpFound && rawOdp) {
                // Jika ODP terlalu ngaco / typo parah sampai ga ketemu di DB
                odpNotFound.push(`${nama} (Diketik: ${rawOdp})`);
            }

            // Push ke antrean save
            dataToSave.push([nowStr, "Instalasi", nama, cid, finalOlt, finalOdp, port, currentUser.nama, ket, email, perangkat, sn]);
        } else {
            errorCount++;
        }
    });
    ...
    // PESAN KONFIRMASI PINTAR
    let confirmMsg = `Ditemukan <b>${dataToSave.length}</b> data laporan valid.<br>`;
    
    if (odpNotFound.length > 0) {
        confirmMsg += `<br><span style="color:#ef4444; font-size:0.85rem; text-align:left; display:block;"><b>Peringatan:</b> Ada ${odpNotFound.length} pelanggan yang ODP-nya gagal dilacak otomatis oleh sistem. Datanya akan tetap dimasukkan apa adanya (silakan edit di Menu Monitoring nanti).</span>`;
    }
    if (errorCount > 0) {
        confirmMsg += `<br><span style="color:#f59e0b; font-size:0.85rem;">Ada ${errorCount} blok teks yang dilewati karena format rusak.</span>`;
    }
    
    confirmMsg += `<br><br>Proses & Simpan sekarang?`;

    // Kita pakai 'html' di SweetAlert agar stylingnya terbaca
    if(!await Swal.fire({ title: 'Konfirmasi Data', html: confirmMsg, icon: 'question', showCancelButton: true }).then(r=>r.isConfirmed)) return;
    ...
    try {
        ...
        if(res.ok) { 
            Swal.fire('Berhasil!', `${dataToSave.length} laporan sukses tersimpan dan terkoreksi otomatis!`, 'success'); 
            fetchData(); 
        } else { 
